...
1. **Установка базы данных:**
   1. Выполнить скрипт `01_Create_Database.sql` в SQL Server
   2. Импортировать данные из CSV файлов в правильной последовательности через SSMS:
      ...
      * В открывшемся окне мастера нажмите **Обзор** и выберите нужный CSV файл
      * Следуйте указаниям мастера, нажимая **Далее** на каждом шаге
      * Повторите процесс для каждого CSV файла
   3. Выполнить `02_Fix_Schema.sql`
...
## Особенности работы с изображениями товаров
...
1. **Тестовые изображения находятся в папке:** 
...
- Путь к рабочей папке `Images`:  
  ...
  ```
- Скопируйте все изображения из `Import\` в созданную папку `Images`
